...
const idOK=id=>/^(?:import-)?[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(id||'');
...
export async function onRequest({request,env}){
 ...
 try{
  ...
  if(action==='song'&&request.method==='PATCH'){
   ...
   const input=JSON.parse(raw),category=input.category,genre=input.genre;
   if(category!==undefined&&category!=='official'&&category!=='creative')return json({error:'譜面の分類が不正です。'},400);
   const genres=['ポップス','アニメ','ボーカロイド','キッズ','バラエティ','クラシック','ゲームミュージック','ナムコオリジナル','その他'];
   if(genre!==undefined&&!genres.includes(genre))return json({error:'ジャンルが不正です。'},400);
   if(category===undefined&&genre===undefined)return json({error:'変更項目がありません。'},400);
   if(category!==undefined){data.songs[index].category=category;data.songs[index].categoryManual=true}
   if(genre!==undefined)data.songs[index].genre=genre;
   await bucket.put(key,JSON.stringify(data),{httpMetadata:{contentType:'application/json'}});
   return json({ok:true,category:data.songs[index].category,genre:data.songs[index].genre});
  }
  if(action==='song'&&request.method==='DELETE'){
   ...
   const [removed]=data.songs.splice(index,1);if(data.songs.length)await bucket.put(key,JSON.stringify(data),{httpMetadata:{contentType:'application/json'}});else{await bucket.delete(key);if(id.startsWith('import-')){await removePrefix(bucket,'uploads/'+id.slice(7)+'/');await bucket.delete('pending/'+id.slice(7)+'.json')}else await removePrefix(bucket,'files/'+id+'/')};return json({ok:true,removed:removed?.title||''});
  }
  if(action==='dan'&&(request.method==='POST'||request.method==='PUT')){
