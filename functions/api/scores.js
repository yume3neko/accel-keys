...
export async function onRequestGet({ env }) {
  try {
    ...
    return json({ error: "ランキングを取得できませんでした。D1の設定をご確認ください。" }, 500);
  }
}
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const score = Number(body.score), maxSpeed = Number(body.maxSpeed), bestCombo = Number(body.bestCombo);
    const mode = body.mode === "hits" ? "hits" : "time";
    if (!name || !Number.isInteger(score) || score < 1 || score > 100000000 ||
        !Number.isFinite(maxSpeed) || maxSpeed < 1 || maxSpeed > 8.1 ||
        !Number.isInteger(bestCombo) || bestCombo < 0 || bestCombo > 1000000) {
      return json({ error: "スコアデータが正しくありません。" }, 400);
    }
    await env.DB.prepare(
      `INSERT INTO scores (player_name, score, mode, max_speed, best_combo) VALUES (?, ?, ?, ?, ?)`
    ).bind(name, score, mode, maxSpeed, bestCombo).run();
    await env.DB.prepare(
      `DELETE FROM scores WHERE id NOT IN (SELECT id FROM scores ORDER BY score DESC, created_at ASC LIMIT 500)`
    ).run();
    const best = await env.DB.prepare(`SELECT MAX(score) AS score FROM scores`).first();
