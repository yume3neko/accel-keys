...
async function start(options={}){
 ...
 resetHibiki(pausedTime);startAt=audioContext.currentTime-pausedTime;scheduleAudio(pausedTime);state='playing';document.body.classList.add('playing');$('overlay').style.display='none';$('pause').disabled=false;$('pause').textContent='一時停止';['course','files','folder','demo','danOpen','danFiles','danFolder'].forEach(id=>$(id).disabled=true);
 update();resize();loop();
}
function togglePause(){if(state==='playing'){pausedTime=time();state='paused';stopAudio();draw();cancelAnimationFrame(raf);$('pause').textContent='再開';$('overlay').style.display='flex';$('overlay').replaceChildren();const h=document.createElement('h2');h.textContent='一時停止';const b=document.createElement('button');b.textContent='▶ 再開';b.className='primary';b.onclick=togglePause;const q=document.createElement('button');q.textContent='選曲に戻る';q.onclick=()=>{if(danRun)exitDan();else{unlock();reset()}};$('overlay').append(h,b,q)}else if(state==='paused'){audio().resume().then(()=>{startAt=audioContext.currentTime-pausedTime;scheduleAudio(pausedTime);state='playing';$('overlay').style.display='none';$('pause').textContent='一時停止';loop()})}}
...
function addRollHits(r,count=1){
