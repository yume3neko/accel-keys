...
function hibikiTravel(c,from,to){
 if(c.hbBpmPoints?.length)return (hbVisualBeatAt(c,to)-hbVisualBeatAt(c,from))*60;
...
function visualBpmState(c,t){
 if(c.hbBpmPoints?.length){const h=hbVisualStateAt(c,t),bpm=Number.isFinite(h.bpm)?h.bpm:(c.bpm||120);return {bpm,hs:1,scroll:1,value:bpm}}
