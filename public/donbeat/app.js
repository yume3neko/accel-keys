...
function isPlayableChartNote(n,c=chart){return !(c?.features?.hbscroll&&Number.isFinite(c?._hbAudioEnd)&&n.time>c._hbAudioEnd+.05)}
...
function hit(type,automatic=false){if(!automatic&&autoInputLocked())return;if(state!=='playing'){if(state==='ready'){audio().resume();tone(type)}return}if(auto&&!automatic)return;tone(type);const t=time()-Number($('offset').value||0)/1000;if(t<judgeFrom)return;const damage=notes.find(n=>n.type===9&&!n.done&&!n.ghost&&Math.abs(n.time-t)<=judgmentWindows().miss);if(damage){judgeCore(damage,1);return}const n=notes.find(n=>!n.done&&!n.ghost&&n.type<=4&&Math.abs(n.time-t)<=judgmentWindows().miss);if(n&&(n.type===1||n.type===3?1:2)===type){judge(n,Math.abs(n.time-t));return}const r=notes.find(n=>!n.done&&!n.ghost&&(n.type>=5&&n.type<=7)&&t>=n.time&&t<=n.end);if(r&&(r.type!==7||type===1))addRollHits(r)}
function songDuration(){if(chart?.features?.hbscroll&&Number.isFinite(chart?._hbAudioEnd))return Math.max(.001,chart._hbAudioEnd);return Math.max(.001,chart.duration+(danRun?0:1),danRun?Math.max(0,...chart.notes.map(n=>(n.end??n.time)+judgmentWindows().miss+.001)):0,audioBuffer?.duration||0)}
...
function loop(){updateBranchRoute();const t=time(),adjust=Number($('offset').value||0)/1000;updateDummyPlayback(t-adjust);for(const n of notes){if(n.done)continue;if(n.type===9){if(t-adjust>n.time+judgmentWindows().miss)n.done=true;continue}if(n.ghost){if(t>(n.end??n.time)+.15)n.done=true;continue}if(n.type<=4){if(auto&&t>=n.time+adjust){pulseAutoPad(n.type===1||n.type===3?1:2,n.type===3||n.type===4);tone(n.type===1||n.type===3?1:2);judge(n,0)}else if(t-adjust>n.time+judgmentWindows().miss)judge(n,1)}else if(auto){const count=autoRollHits(n,t-adjust)-n.hits;if(count>0){addRollHits(n,count);pulseAutoPad(n.type===7?1:n.hits%2+1);tone(n.type===7?1:n.hits%2+1)}}}if(danRun&&!danRun.failed&&danCurrentFailed()){danRun.failed=true;updateDanHUD()}$('status').textContent=danRun?.failed?'不合格確定 · この曲の終了まで演奏できます':!auto&&t-adjust<judgeFrom?'助走中 · 判定なし → '+judgeFrom.toFixed(2)+'秒から':auto?'オートプレイ中':practiceTarget===null?'演奏中':'途中から演奏中';draw();if(t>songDuration()){finish();return}raf=requestAnimationFrame(loop)}
...
function soulNoteCount(){return Math.max(1,danRun?danRun.config.songs.reduce((sum,e)=>sum+e.chart.notes.filter(n=>n.type<=4&&isPlayableChartNote(n,e.chart)).length,0):chart.notes.filter(n=>n.type<=4&&isPlayableChartNote(n,chart)).length)}
...
function hibikiTravel(c,from,to){
 if(c.hbSegments?.length)return (hbVisualBeatAt(c,to)-hbVisualBeatAt(c,from))*60;
...
function renderSongSelection(){
 ...
 for(const [title,entries] of songGroups()){
  ...
  if(opened){
   ...
   info.textContent=c.bpm+' BPM ／ '+Math.floor(duration/60)+':'+String(Math.floor(duration%60)).padStart(2,'0')+' ／ '+c.notes.filter(n=>n.type<=4&&isPlayableChartNote(n,c)).length+' ノーツ';panel.append(info);
...
function visualBpmState(c,t){
 if(c.hbSegments?.length){const h=hbVisualStateAt(c,t),bpm=Number.isFinite(h.bpm)?h.bpm:(c.bpm||120),scroll=Number.isFinite(h.scroll)?h.scroll:1;return {bpm,hs:1,scroll,value:bpm*scroll}}
