...
function danCurrentFailed(){
 if(!danRun||danRun.config.festival)return false;if(danRun.failed||danExceededMaximum())return true;
...
function updateDanHUD(){updateSoulDisplay();const hud=$('danHud');if(!danRun){hud.hidden=true;return}hud.hidden=false;hud.replaceChildren();hud.append(danNode('b',danRun.config.name+'　'+(danRun.index+1)+'/'+danRun.config.songs.length+'曲　'+(danRun.config.festival?'ライブフェスティバル':danRun.failed?'不合格確定・この曲まで':''),'dan-heading'));
 if(danRun.config.festival){hud.append(danNode('p','通しスコア '+Number(score||0).toLocaleString()+' ／ 良 '+good+' ／ 可 '+ok+' ／ 不可 '+miss,'dan-preview-status'));return}
...
async function launchFestival(festival,automatic=false){
 ...
 for(const e of songs)if(!charts.includes(e.chart))charts.push(e.chart);
 fillCourses();$('course').value=String(charts.indexOf(songs[0].chart));
...
function finishDanSong(){if(!danRun||danRun.finished)return;pausedTime=time();stopAudio();cancelAnimationFrame(raf);danRun.results.push(danSongStats());const complete=danRun.index===danRun.config.songs.length-1,result=danRun.config.festival?{pass:true}:evaluateDan(danRun.config,danRun.results,danStats(),soul,complete);if(danRun.failed||!result.pass||complete){showDanResult(danRun.failed||!result.pass);return}state='dan-break';$('pause').disabled=true;queueDanNext();}
...
var danTransitionBusy=false,danSceneAnimations=[];
function waitDanVisible(){if(!document.hidden)return Promise.resolve();return new Promise(resolve=>{const onVisible=()=>{if(!document.hidden){document.removeEventListener('visibilitychange',onVisible);resolve()}};document.addEventListener('visibilitychange',onVisible)})}
function clearDanSceneFade(){for(const a of danSceneAnimations)a.cancel();danSceneAnimations=[];danTransitionBusy=false;}
async function fadeDanScene(enter){
 const nodes=document.querySelectorAll('.scorebar,#courseProgress,#songProgress,.lane');
 const previous=danSceneAnimations;
 danSceneAnimations=[...nodes].map(n=>n.animate([{opacity:enter?0:getComputedStyle(n).opacity},{opacity:enter?1:0}],{duration:1000,easing:'ease-in-out',fill:'forwards'}));
 previous.forEach(a=>a.cancel());
 const current=danSceneAnimations;
 await Promise.all(current.map(a=>a.finished.catch(()=>{})));
 if(enter&&danSceneAnimations===current){for(const a of current)a.cancel();danSceneAnimations=[]}
}
async function queueDanNext(){
 if(danTransitionBusy||!danRun||state!=='dan-break')return;
 danTransitionBusy=true;const run=danRun;
 try{
  await waitDanVisible();if(danRun!==run||run.finished)return;
  $('status').textContent='次の曲へ';
  await fadeDanScene(false);await waitDanVisible();if(danRun!==run||run.finished)return;
  run.index++;await beginDanSong(true);
 }finally{if(danRun===run)danTransitionBusy=false}
}
