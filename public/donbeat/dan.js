...
async function launchDan(automatic=false){if(loading||importing||danRun)return;let config;try{config=resolveDanConfig();config.auto=automatic===true}catch(e){$('danError').textContent=e.message;return}loading=true;enterPlayFullscreen();$('danPlay').disabled=true;$('danClose').disabled=true;try{await audio().resume();const buffers=[],cache=new Map();for(let i=0;i<config.songs.length;i++){const e=config.songs[i],c=e.chart;$('danError').textContent=(i+1)+'/'+config.songs.length+'曲目の音源を準備中…';if(e.demo){buffers.push(null);continue}const file=c.audioFile;if(!file)throw Error((i+1)+'曲目の音源がありません。対応する音源を追加してください。');if(c===chart&&audioBuffer)cache.set(file,audioBuffer);if(!cache.has(file))cache.set(file,c.preloadedAudio||await audio().decodeAudioData(await file.arrayBuffer()));buffers.push(cache.get(file))}
