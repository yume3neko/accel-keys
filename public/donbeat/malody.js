...
function parseMalody(text){
 ...
 const map=[1,3,2,4,5,6,7,9],notes=[];let lastBeat=0;
 ...
 if((type>=5&&type<=7)||type===9){if(!n.endbeat)throw Error('連打・風船・くす玉の終点がありません。');const end=beat(n.endbeat);if(end<b)throw Error('連打の終点が始点より前です。');note.end=seconds(end)-offset;note.hits=0;lastBeat=Math.max(lastBeat,end);if(type===7||type===9){note.required=Number(n.hits);if(!Number.isInteger(note.required)||note.required<1)throw Error((type===9?'くす玉':'風船')+'の必要打数が不正です。')}}notes.push(note)}
