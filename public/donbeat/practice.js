 ...
 for(let row=begin;row<Math.min(total,begin+4);row++){
  ...
  for(let side=0;side<2;side++){
   ...
   for(const n of chart.notes){if(n.time>=m.end||(n.end??n.time)<m.time)continue;const px=x(n.time),radius=n.type===3||n.type===4||n.type===6?r*1.3:r;
    if(n.type===5||n.type===6||n.type===7){c.strokeStyle=n.type===7?'#cf6d59':'#efc255';c.lineWidth=radius*1.5;c.lineCap='round';c.beginPath();c.moveTo(Math.max(left,px),y);c.lineTo(Math.min(left+half,x(n.end)),y);c.stroke()}
    if(n.time<m.time)continue;c.fillStyle=n.type===10?'#b569eb':n.type===7?'#df765b':(n.type===9||n.type===5||n.type===6)?'#efc255':n.type===1||n.type===3?'#fa725a':'#5dc5e3';c.strokeStyle='#f5eee0';c.lineWidth=1.2;c.beginPath();c.arc(px,y,radius,0,Math.PI*2);c.fill();c.stroke();
   }
