...
function hbBpmPointAt(chart,t){
 ...
 if(!points.length)return {time:0,beat:0,bpm:chart?.bpm||120,order:0};
 let last=points[0];
 const now=hbMs(t);
 for(let i=1;i<points.length;i++){
  const p=points[i],next=points[i+1];
  const after=now>=hbMs(p.time);
  const before=!next||now<hbMs(next.time);
  if(after&&before)last=p;
 }
 return last;
}
function hbVisualStateAt(chart,t){
 const p=hbBpmPointAt(chart,t);
 return {beat:p.beat+(t-p.time)*p.bpm/60,bpm:p.bpm,point:p};
}
