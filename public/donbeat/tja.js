...
function hbSegmentAt(chart,t){
 ...
 for(let i=segments.length-1;i>=0;i--){
  const e=segments[i],lo=Math.min(e.startTime,e.endTime),hi=Math.max(e.startTime,e.endTime);
  if(t>=lo-1e-9&&t<=hi+1e-9)return e;
 }
 return null;
}
function hbVisualStateAt(chart,t){
 ...
 if(e){
  ...
  return {beat:e.startBeat+(e.endBeat-e.startBeat)*ratio,bpm:e.bpm,segment:e};
 }
 const initialTime=chart?.hbInitialTime??0,initialBeat=chart?.hbInitialBeat??0,initialBpm=chart?.hbInitialBpm??chart?.bpm??120;
 return {beat:initialBeat+(t-initialTime)*initialBpm/60,bpm:initialBpm,segment:null};
}
