...
function branchReferenceScoreBasis(chart){
 if(!chart)return {notes:1,requiredHits:0};
 if(chart._branchScoreBasis)return chart._branchScoreBasis;
 let source=chart;
 if(chart._tja&&chart.branchEvents?.length){
  try{source=compile({...chart._tja.meta},chart._tja.lines,new Set(),new Array(chart.branchEvents.length).fill('M'))}catch{}
 }
 const notes=Math.max(1,(source.notes||[]).filter(n=>n.type<=4).length);
 const requiredHits=(source.notes||[]).reduce((sum,n)=>sum+((n.type===7||n.type===9)&&Number.isFinite(n.required)?n.required:0),0);
 chart._branchScoreBasis={notes,requiredHits};
 return chart._branchScoreBasis;
}
...
root.branchReferenceScoreBasis=branchReferenceScoreBasis;
root.chartNoteScore=chartNoteScore;
...
if(typeof module!=='undefined')module.exports={parseTJA,branchReferenceScoreBasis,chartNoteScore,branchReferenceNoteCount,branchNoteCountRange};
