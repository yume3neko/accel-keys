...
function hbBpmPointAt(chart,t){
 const points=chart?.hbBpmPoints||[];
 if(!points.length)return {time:chart?.hbInitialTime??0,beat:chart?.hbInitialBeat??0,bpm:chart?.hbInitialBpm??chart?.bpm??120};
 let last=points[0];
...
function hbVisualStateAt(chart,t){
 const p=hbBpmPointAt(chart,t);
 return {beat:p.beat+(t-p.time)*p.bpm/60,bpm:p.bpm,scroll:1,point:p};
}
