...
test('reserved substrings and invisible/whitespace variants are rejected even with an additional password',()=>{
 for(const n of ['ゆめみねこ','偽ゆめみねこ123','prefix ゆめみねこ suffix','ゆ めみねこ','ゆめ\u200bみねこ','ゆめみねこ(製作者)']){assert.ok(reservedName(n));assert.throws(()=>identifyName(n,'wrong',{ADMIN_TOKEN:secret}),{status:400});assert.throws(()=>identifyName(n,secret,{ADMIN_TOKEN:secret}),{message:'この名前は使えません',status:400});}
...
test('normal multiplayer password names are converted before persistence; spoof flags denied',async()=>{
 ...
 assert.equal((await call(guest,{action:'join',name:'xxゆめみねこ',creator:true})).code,'NAME_NOT_ALLOWED');
 const b=await call(guest,{action:'join',name:'xxゆめみねこ',adminPassword:secret});assert.equal(b.status,400);
 const valid=await call(guest,{action:'join',name:secret});assert.equal(valid.players.find(p=>p.id===valid.you).creator,true);
...
test('both rankings protect reserved names and expose legacy badge without changing scores',async()=>{
 ...
 assert.equal(await post('xゆめみねこx'),400);assert.equal(await post('xゆめみねこx',secret),400);assert.equal(await post(secret),201);
